...
chr_theme theme uses [Webpack](https://webpack.js.org) to compile and
bundle SASS and JS.
...
Go to the root of chr_theme theme and run the following commands:
`nvm use`
...
`drush en chr_theme -y`
`drush config-set system.theme default chr_theme -y`
